...
def decrypt_password(encrypted_password: str) -> str | Exception:
    ...
    try:
        ...
        aes_obj = AES.new(eval(key), AES.MODE_ECB)
        hex_dec = base64.b64decode(encrypted_password.encode("latin_1"))
        bytes_decoded = aes_obj.decrypt(hex_dec)
        str_decoded_pwd = bytes_decoded.strip().decode("latin_1")
        return str_decoded_pwd
    except Exception as e:
...
class Security:
    ...
    def open_aws_session(
        ...
        try:
            if Security.config_utils.get_bool_decrypt_test_password():
