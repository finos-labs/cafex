...
class UISecurity(Security):
    ...
    def start_bs_local(self, bs_local_obj: Local, local_args: dict) -> Local | Exception:
        """Starts BrowserStack Local Server."""
        try:
            if self.config_utils.get_bool_decrypt_test_password():
    ...
    def __create_browserstack_url(self, user_name: str, access_key: str) -> str | None:
        try:
            if self.config_utils.get_bool_decrypt_test_password():
